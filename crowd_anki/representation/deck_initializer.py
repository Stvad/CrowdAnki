...
def from_collection(collection, config: ConfigSettings, name, deck_metadata=None, is_child=False) -> Deck:
    ...
    deck.notes = Note.get_notes_from_collection(collection, config, deck.anki_dict["id"], deck.metadata.models)
    ...
    deck.children = seq(direct_children) \
        .map(lambda child_name: from_collection(collection, config, child_name, deck.metadata, True)) \
        .filter(lambda it: it is not None).to_list()

    return deck


def from_json(json_dict, config: ConfigSettings, deck_metadata=None) -> Deck:
    ...
    deck.notes = [Note.from_json(json_note, config) for json_note in json_dict["notes"]]
    deck.children = [from_json(child, config, deck.metadata) for child in json_dict["children"]]
